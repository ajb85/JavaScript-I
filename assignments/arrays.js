...
// !!! JOSH REMOVED THESE RESTRICTIONS IN WEB17 CHAT !!!
// Example for loop:
// arr = [1,2,3,4];
...
const car = findCarID(33);
function findCarID(carID) {
...
console.log(
  `Challenge 1: Car ${car.id} is a ${car.car_year} ${car.car_make} ${
    car.car_model
  }`
);
...
console.log(
  `Challenge 2: Car ${lastCar.id} is a ${lastCar.car_make} ${lastCar.car_model}`
);
...
console.log("Challenge 3: ", getCarInfo("car_model").join(", "));

// ==== Challenge 4 ====
// The accounting team needs all the years from every car on the lot. Create a
// new array from the dealer data containing only the car years and log the result in the console.
console.log("Challenge 4: ", getCarInfo("car_year").join(", "));
...
console.log(
  "Challenge 5: ",
  getCarInfo("car_year").filter(year => year < 2000).length
);
...
const bmwAndAudi = inventory.filter(
  car =>
    car.car_make.toLowerCase() === "audi" ||
    car.car_make.toLowerCase() === "bmw"
);
...
function getCarInfo(info) {
  ...
  return typeof inventory[0][info] === "string"
    ? bubbleSortArr(cars)
    : cars.sort((a, b) => a - b);
}

function bubbleSortArr(arr) {
  // .sort() doesn't know how to compare uppercase to lowercase because it converts to unicode
  // and uppercase letters appear in unicode before lowercase.  This will sort
  // regardless of case.
  ...
  while (cycle) {
    // "arrChanged" lets me know if the 'for' loop made no changes.  If no changes,
    // it is sorted
    let arrChanged = false;
    for (let i = 0; i < arr.length; i++) {
      // Variable shortcuts
      const word1 = sortedArr[i];
      const word2 = sortedArr[i + 1];
      // If letters are out of order, set 'arrChanged' and resort the string
      if (word2 && word1.toLowerCase() > word2.toLowerCase()) {
        arrChanged = true;
        sortedArr[i] = word2;
        sortedArr[i + 1] = word1;
      }
